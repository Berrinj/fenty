...
export async function renderPostCarousel() {
  try {
    ...
    const featuredMedia =
      displayPost[currentPostIndex]._embedded["wp:featuredmedia"] &&
      displayPost[currentPostIndex]._embedded["wp:featuredmedia"][0] &&
      displayPost[currentPostIndex]._embedded["wp:featuredmedia"][0].source_url;
    const imageUrl = featuredMedia || "/img/RIHANNAnm.jpg";
    const featuredMediaAlt =
      displayPost[currentPostIndex]._embedded["wp:featuredmedia"] &&
      displayPost[currentPostIndex]._embedded["wp:featuredmedia"][0] &&
      displayPost[currentPostIndex]._embedded["wp:featuredmedia"][0].alt_text;
    const imageAltText = featuredMediaAlt || `missing alt text`;
    const postUrl = getPostUrl(
      displayPost[currentPostIndex].id,
      displayPost[currentPostIndex].slug,
    );
    postCarousel.innerHTML = `<a href="${postUrl}"><img class="carousel-img" src="${imageUrl}" alt="${imageAltText}"> <p class="carousel-text">${displayPost[currentPostIndex].title.rendered}</p>`;
...
export function updateArrowButtonStates() {
  ...
  rightArrow.classList.toggle(
    "disabled-arrow",
    currentPostIndex === displayPost.length - 1,
  );
}
